...
public: // standard Operations
	...
	friend wostream & operator<< <T, maxDirections>(wostream &os, DagPosition1& p);
	...
	)
{
	...
	if (height == 0) {
		...
		os << L"([" << p->label << L" H: " << right << setw(2) << height << L"]";
		for (int direction = 0; direction < maxDirections; direction++) {
			os << L",";
...
template <class T, int maxDirections>
wostream & operator<<(wostream &os, DagPosition1<T, maxDirections>& p)
{
