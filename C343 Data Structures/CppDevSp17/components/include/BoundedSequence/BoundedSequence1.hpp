...
{
	contents[currentLength] * sToAppend.contents;
...
void BoundedSequence1<T, maxLength>::split(Integer pos, BoundedSequence1& receivingS)
{
	T temp;
	for (int i = 0; i < pos; i++) {
		temp.add(temp.currentLength, contents[i])
	}
	receivingS.clear();
	for (int i = pos; i  <= currentLength; i ++)
	{
		receivingS.add(receivingS.currentLength, contents[i]);
	}
	contents = temp;
}	// split
