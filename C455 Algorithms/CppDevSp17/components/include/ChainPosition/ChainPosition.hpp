...
public:
	...
	friend wostream & operator<< <T>(wostream &os, ChainPosition1& p);
...
template <class T>
void ChainPosition1<T>::swapWithTarget (ChainPosition1& target)
{
...
template <class T>
Boolean ChainPosition1<T>::operator == (ChainPosition1& rhs)
{
	return rep == rhs.rep;
}	// operator ==
...
template <class T>
wostream & operator<<(wostream &os, ChainPosition1<T>& p)
{
	...
	while(!localP.isZero()) {
		if(localP.isZero()) {
			...
			os << L"#" << localP.rep << L":" << L"(" << localP.rep->label << L"," << (localP.rep->refCount - 1) << L",#";
			ChainPosition1<T> nextP = localP;
			nextP.applyTarget();			
			if (nextP.isZero()) {
				os << L"0)";
			} else {
				os << localP.rep->target << L")";
			} // end if			
		} // end while
		os << endl;
		localP.applyTarget();
